...
def get_dataframes_from_journal(save=False):
    try:
...
def process_checks_cut(cost_center_replacements, do_transfer):
    ...
    dfs, file_name, filtered, masks, re_issue, fiscal_month, fiscal_year = get_dataframes_from_journal(save=False)
...
def get_checks_cut_path(base_dir):
    ...
    for dir in dirs:
        ...
        if "temp" in dir_lowered and "_copy" in dir_lowered and ".xlsx" in dir_lowered:
